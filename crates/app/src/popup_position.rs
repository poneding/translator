...
///
/// The result is always clamped to be fully visible inside `screen`.
pub async fn compute_popup_position(monitor: &dyn SelectionMonitor, screen: Rect) -> (i32, i32) {
    // Try selection bounds first.
    if let Ok(Some(sel)) = monitor.selection_bounds().await {
        let (x, y) = position::below_rect(sel, POPUP_GAP);
        return position::clamp_to_screen(x, y, DEFAULT_POPUP_W, DEFAULT_POPUP_H, screen);
    }
    // Fall back to cursor position.
    if let Ok((cx, cy)) = monitor.cursor_position().await {
        let (x, y) = (cx, cy + POPUP_GAP);
        return position::clamp_to_screen(x, y, DEFAULT_POPUP_W, DEFAULT_POPUP_H, screen);
    }
...
mod tests {
    ...
    async fn uses_selection_bounds_when_available() {
        ...
        let (x, y) = compute_popup_position(monitor.as_ref(), full_hd_screen()).await;
        assert_eq!(x, 100);
        assert_eq!(y, 232);
    }
    ...
    async fn clamps_to_screen_bottom_edge() {
        ...
        assert_eq!(x, 1440);
        // 1000 + 20 + 12 = 1032 + 320 = 1352 > 1080 -> y = 1032 - 320 = 712
        assert_eq!(y, 712);
    }
}
