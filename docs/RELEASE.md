...
macOS releases use a fixed self-signed code-signing certificate. This does not
require a paid Apple Developer Program account, but the same certificate must be
reused for future releases so macOS sees the app as the same signing identity.

GitHub Actions secrets:

- `MACOS_CODESIGN_CERTIFICATE` — base64 of the fixed self-signed `.p12`
- `MACOS_CODESIGN_CERTIFICATE_PASSWORD` — password for the `.p12`
- `MACOS_CODESIGN_IDENTITY` — certificate common name, defaults to
  `Translator Local Code Signing`

Generate the certificate once and keep the `.p12` private:

```bash
CERT_PASSWORD='<strong-password>' \
scripts/macos/create-self-signed-codesign-cert.sh \
  ./translator-codesign.p12 \
  "Translator Local Code Signing"
```

The script prints the three GitHub secret values to add under
...
Settings while the new binary is denied at runtime. A fixed self-signed
certificate gives the app a stable designated requirement without a Developer ID
certificate.

For local verification or re-signing an installed app:

```bash
APPLE_SIGNING_IDENTITY="Translator Local Code Signing" \
scripts/macos-sign-app.sh /Applications/Translator.app

# Local development/self-use when no release certificate is installed.
...
Future hardening:
- Replace the self-signed certificate with a Developer ID Application
  certificate when Gatekeeper/notarization is required.
...
2. Add the matching private key string printed by `cargo tauri signer generate`
   to GitHub Secrets as `TAURI_SIGNING_PRIVATE_KEY_B64`, encoded one more time
   as single-line base64:
...
3. Add the private key password to GitHub Secrets as
   `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` if the key was generated with a
   password.
...
- **"Missing comment in secret key"** — `TAURI_SIGNING_PRIVATE_KEY_B64` was
  encoded from the wrong value. Decode it once locally and verify the result is
  the private key line printed by `cargo tauri signer generate`; decoding that
  line should reveal an `untrusted comment` header.
