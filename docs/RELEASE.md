...
3. Add the private key password to GitHub Secrets as
   ...
   without a password).
4. Set `bundle.createUpdaterArtifacts` to `true` in
   `crates/app/tauri.conf.json`.
5. Verify the release draft includes the updater manifest and signatures.
6. Test Settings -> Update against the published manifest before announcing
   the release.
